  ...
  return (
    <div className="flex-1 flex flex-col bg-[var(--background)] min-h-full">
      ...
      <main className="flex-1 max-w-md w-full mx-auto px-4 pt-[max(0.75rem,env(safe-area-inset-top))] pb-32 scroll-momentum">
        ...
        <p className="text-[15px] text-[var(--text-tertiary)] mt-1 mb-4">Everything happening across your groups</p>

        {allItems.length === 0 ? (
          <p className="text-center text-[var(--text-tertiary)] text-sm py-20">No activity yet.</p>
        ) : (
            ...
            <div className="space-y-1">
              {rows.map(({ item, bucket, showBucket }) => (
                <div key={item.key}>
                  ...
                  >
                    <span className={`relative z-[1] w-9 h-9 rounded-full flex items-center justify-center shrink-0 ${item.kind === "settlement" ? "bg-[var(--tint-success)]" : "bg-[var(--tint-accent-2)]"}`}>
                    ...
                    <span className="text-[12px] text-[var(--text-tertiary)] shrink-0">{timeLabel(item.ts)}</span>
                  </button>
                </div>
              ))}
            </div>
